...
To run the tests $ python testMoltenProt.py

We are testing that the MoltenProt class can load all the example files
...
moltenProt_relative_path   =  "../../foldA_moltenP_apps/moltenprot"
...
sys.path.append(moltenProt_relative_path)

from testHelpers      import *
from moltenprot_shiny import DSF_molten_prot_fit
...
class testLoadFiles(unittest.TestCase):
    ...
    def testLoad(self,dsf=None):
        ...
        self.assertTrue(max(dsf.temps)>200, 'check that the data is in kelvin' )    
...
class testRequiredPackages(unittest.TestCase):
    ...
    def test_R_packages(self):
        ...
        _ , files = run_fast_scandir(moltenProt_relative_path,".r")
        ...
        for p in pkgs:
            if p not in pkgs_to_be_installed:
                print(p)
    ...
    def test_python_packages(self):
        ...
        """
        Test that all the python required packaged are going to be installed

        """

        _ , files = run_fast_scandir(moltenProt_relative_path,".py")
        pkgs =  []
        ...
        # codecs is installed automatically
...
if __name__ == '__main__':
    print("Starting MoltenProt tests")
    unittest.main()
