fit_two_sites_two_kd_different_signal <- function(signal,lig_conc,bt,
  ...
  kds     <- unique(2**lseq)
  f1s     <- unique(seq(min_rf1,max_rf1,length.out = 9))
  f2s     <- unique(seq(min_rf2,max_rf2,length.out = 9))
