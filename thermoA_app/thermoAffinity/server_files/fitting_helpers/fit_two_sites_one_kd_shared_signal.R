fit_two_sites_one_kd_shared_signal <- function(signal,lig_conc,bt,
  ...
  # Set the minimum and maximum values for RF1, RF2, and Kd based on the fixed values
  if (fix_rf1) (  min_rf1 <- max_rf1 <- rf1_init )
  if (fix_rf2) (  min_rf2 <- max_rf2 <- rf2_init )
  if (fix_kd)  (  min_kd  <- max_kd  <- kd_init  ) 
  
  # Define the fluorescence signal function
  fluo_eq <-  function(at,RF1,RF2,Kd) {
  ...
  # Find initial parameters for optimization
  errors  <- c()
  ...
  # Create a data frame with errors and parameter values
